...
namespace AgOpenGPS
{
    public partial class CGuidance
    {
        ...
        public void BuildCurrentABLineList(vec3 pivot)
        {
            ...
            if (selectedABLine?.curvePts.Count > 1)
            {
                //x2-x1
                dy = selectedABLine.curvePts[1].easting - selectedABLine.curvePts[0].easting;
                //z2-z1
                dx = selectedABLine.curvePts[1].northing - selectedABLine.curvePts[0].northing;

                double heading = Math.Atan2(dy,dx);
                double distanceFromRefLine = ((dx * mf.guidanceLookPos.easting) - (dy * mf.guidanceLookPos.northing) + (selectedABLine.curvePts[1].easting
                                        * selectedABLine.curvePts[0].northing) - (selectedABLine.curvePts[1].northing * selectedABLine.curvePts[0].easting))
                ...
                if (!isValid || howManyPathsAway != oldHowManyPathsAway)
                {
                    ...
                    vec2 point1 = new vec2(selectedABLine.curvePts[0].easting + Math.Cos(-heading) * distAway, selectedABLine.curvePts[0].northing + Math.Sin(-heading) * distAway);
        ...
        public void DrawABLines()
        {
            ...
            if (selectedABLine?.curvePts.Count > 1)
            {
                ...
                GL.Vertex3(selectedABLine.curvePts[0].easting, selectedABLine.curvePts[0].northing, 0.0);
                GL.End();

                if (mf.font.isFontOn && !isABLineBeingSet)
                    mf.font.DrawText3D(selectedABLine.curvePts[0].easting, selectedABLine.curvePts[0].northing, "&A");
                ...
                double heading = Math.Atan2(selectedABLine.curvePts[1].easting - selectedABLine.curvePts[0].easting, selectedABLine.curvePts[1].northing - selectedABLine.curvePts[0].northing);

                GL.Begin(PrimitiveType.Lines);
                  GL.Vertex3(selectedABLine.curvePts[0].easting - (Math.Sin(heading) * abLength), selectedABLine.curvePts[0].northing - (Math.Cos(heading) * abLength), 0);
                  GL.Vertex3(selectedABLine.curvePts[1].easting + (Math.Sin(heading) * abLength), selectedABLine.curvePts[1].northing + (Math.Cos(heading) * abLength), 0);
                GL.End();
        ...
        public void BuildTram()
        {
            ...
            if (selectedABLine?.curvePts.Count > 1)
            {
                ...
                {
                    P1.easting = selectedABLine.curvePts[0].easting + (hsin * i);
                    P1.northing = selectedABLine.curvePts[0].northing + (hcos * i);
                    tramRef.Add(P1);
                }
        ...
        public void SetABLineByHeading(double heading)
        {
            if (selectedABLine?.curvePts.Count > 1)
            {
                selectedABLine.curvePts[0] = new vec3(selectedABLine.curvePts[0].easting, selectedABLine.curvePts[0].northing, heading);

                selectedABLine.curvePts[1] = new vec3(selectedABLine.curvePts[0].easting + Math.Sin(heading), selectedABLine.curvePts[0].northing + Math.Cos(heading), heading);
            }
        }
        ...
        public void MoveABLine(double dist)
        {
            if (selectedABLine?.curvePts.Count > 1)
            {
                ...
                selectedABLine.curvePts[0] = new vec3(selectedABLine.curvePts[0].easting + Math.Cos(heading) * (isHeadingSameWay ? dist : -dist),
                    selectedABLine.curvePts[0].northing - Math.Sin(heading) * (isHeadingSameWay ? dist : -dist), heading);
                selectedABLine.curvePts[1] = new vec3(selectedABLine.curvePts[0].easting + Math.Sin(heading), selectedABLine.curvePts[0].northing + Math.Cos(heading), heading);
            }
            isValid = false;
        }
